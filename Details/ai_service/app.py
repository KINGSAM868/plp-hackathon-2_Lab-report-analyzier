...
def verify_payment():
    """Endpoint to handle and verify a payment from Intasend."""
    data = request.json
    results = data.get("results")
    submission_id = data.get("submission_id")

    if not all([results, submission_id]):
        return jsonify({"msg": "Missing required payment data"}), 400
    
    conn = get_db_connection()
    cursor = conn.cursor()
    try:
        # Check if the payment was successful based on Intasend's results
        if results.get("status") == "SUCCESS":
            # NOTE: You would need to add an 'is_paid' column to your submissions table
            # ALTER TABLE submissions ADD COLUMN is_paid BOOLEAN DEFAULT FALSE;
            
            # Update the submission in the database to mark it as paid
            cursor.execute("UPDATE submissions SET is_paid = TRUE WHERE id = %s", (submission_id,))
            conn.commit()

            # For demonstration, let's assume we get the student's phone number
            # from another source or the submission itself.
            # In a real app, you would fetch this from the database based on submission_id
            student_phone_number = "254712345678" 
            send_message_notification(student_phone_number, "Your payment was successful and your lab report is now ready for grading.")
            
            return jsonify({"msg": "Payment verified and submission updated successfully"}), 200
        else:
